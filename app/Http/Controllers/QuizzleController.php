...
{
    // Get random quiz
    $randomQuiz = Quiz::with(['questions', 'questions.answers'])->inRandomOrder()->first();

    // If no quizzes are available, return to home with an error
    if (!$randomQuiz) {
        return redirect()->route('home')->with('error', 'No quizzes available.');
    }

    // Start quiz session
    session(['quiz' => $randomQuiz]);
    ...
    public function nextQuestion()
{
    ...
    // Provide a default value when getting 'question_number' from session
    $question_number = Session::get('question_number', 0);

    // Check if 'question_number' is less than the number of questions in 'quiz'
    if ($question_number < count($quiz->questions)) {

        // Get the next question from 'quiz', handle collection like collection not array
        $nextQuestion = $quiz->questions->get($question_number);
